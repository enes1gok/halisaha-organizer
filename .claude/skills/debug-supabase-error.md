...
description: Supabase/Postgres hatalarını adım adım teşhis etmek için rehber (SQLSTATE, RLS, RPC). Başarısız RPC, RLS reddi veya mapSupabaseError kaynaklı AppError araştırmalarında kullan.
---
...
- If `traceId` is present, search logs for that value to correlate Edge Function or RPC logs.

## Related

- Error mapping rules: [.claude/rules/error-handling-governance.md](.claude/rules/error-handling-governance.md)
- RLS security model: [.claude/rules/supabase-governance.md](.claude/rules/supabase-governance.md)
- Function signature issues (42P13): [.claude/skills/evolve-postgres-function.md](.claude/skills/evolve-postgres-function.md)
