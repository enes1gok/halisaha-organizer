...
1. **Theme:** use [src/theme/index.ts](src/theme/index.ts) (`colors`, `spacing`, `typography`, `radius`) — see [.claude/rules/ui-ux-design.md](.claude/rules/ui-ux-design.md).
...
- Read/write match and player data through domain hooks from [src/store/index.ts](src/store/index.ts) (`useMatchesStore`, `usePlayersStore`, …) with **atomic selectors** and `useShallow` when selecting multiple actions/fields — see [.claude/rules/technical-excellence.md](.claude/rules/technical-excellence.md).

## When behavior touches domain rules

If the screen adds fields, status transitions, or persistence, follow [.claude/skills/add-domain-feature.md](.claude/skills/add-domain-feature.md) as well.
