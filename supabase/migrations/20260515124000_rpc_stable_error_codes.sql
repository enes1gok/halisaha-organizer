...
begin
  ...
  if coalesce(locked, false) then
    raise exception 'ERR_MATCH_LINEUP_LOCKED';
  end if;
...
begin
  if p_group_id is not null and not public.can_view_group(p_group_id, auth.uid()) then
    raise exception 'ERR_GROUP_LEADERBOARD_FORBIDDEN';
  end if;
...
begin
  ...
  if char_length(trimmed) < 2 then
    raise exception 'ERR_GROUP_NAME_MIN';
  end if;
  if char_length(trimmed) > 80 then
    raise exception 'ERR_GROUP_NAME_MAX';
  end if;
...
begin
  ...
  if not public.match_rating_rater_can_participate(p_match_id, uid) then
    raise exception 'ERR_RATING_CANNOT_PARTICIPATE';
  end if;

  select m.status into st from public.matches m where m.id = p_match_id;
  if st is null then
    raise exception 'ERR_MATCH_NOT_FOUND';
  end if;
  if st <> 'finished'::public.match_status then
    raise exception 'ERR_RATING_FINISHED_ONLY';
  end if;
  ...
  loop
    ...
    if pid is null or pid = uid then
      raise exception 'ERR_RATING_INVALID_RATEE';
    end if;
    if sc is null or sc < 1 or sc > 10 then
      raise exception 'ERR_RATING_SCORE_RANGE';
    end if;
    if not public.match_rating_ratee_is_eligible(p_match_id, pid) then
      raise exception 'ERR_RATING_RATEE_INELIGIBLE';
    end if;
  end loop;
...
begin
  ...
  if not public.match_rating_rater_can_participate(p_match_id, uid) then
    raise exception 'ERR_MOTM_CANNOT_VOTE';
  end if;

  if p_pick_player_id is null or p_pick_player_id = uid then
    raise exception 'ERR_MOTM_INVALID_PICK';
  end if;
  ...
  if st <> 'finished'::public.match_status then
    raise exception 'ERR_MOTM_FINISHED_ONLY';
  end if;

  if not public.match_rating_ratee_is_eligible(p_match_id, p_pick_player_id) then
    raise exception 'ERR_MOTM_PLAYER_NOT_ON_FIELD';
  end if;
