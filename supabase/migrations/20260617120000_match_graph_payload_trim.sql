-- Trim match graph payloads: explicit match columns, omit timestamps on wire,
-- stat_lines only for finished matches, slimmer self_reports JSON,
-- series_id / spawned_from_match_id with visibility rules,
-- list hydrate fast path via match_graph_row_summary_body (no stat_lines / self_reports).
...
-- ---------------------------------------------------------------------------
-- Summary body (list hydrate): attendees + teams + profiles only
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- match_graph_row wrapper (visibility)
-- ---------------------------------------------------------------------------
...
create or replace function public.list_visible_match_summaries_for_user(p_limit integer default null)
returns table (
  id uuid,
  starts_at timestamptz,
  venue text,
...
revoke execute on function public.list_visible_match_summaries_for_user(integer) from public;
revoke execute on function public.list_visible_match_summaries_for_user(integer) from anon;
grant execute on function public.list_visible_match_summaries_for_user(integer) to authenticated;
