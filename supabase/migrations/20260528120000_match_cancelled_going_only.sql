-- Narrow match cancellation push audience to going attendees (RSVP='going') only.
-- Previously the trigger fanned out to every group member; product spec is to notify
-- only the players who said "katılıyorum". Organizer is still excluded.
-- Edge buildMessage / preference key (group_match_cancelled) and the match_cancelled
-- dedup index remain unchanged.
